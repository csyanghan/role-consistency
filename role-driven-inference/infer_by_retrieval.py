...
def retrieval_role_vote(roles):
    role_vote = {}
    for role in roles:
        for r in role:
            if r in role_vote:
                role_vote[r] += 1 if args.role_vote == "normal" else (1 / len(role)) 
            else:
                role_vote[r] = 1 if args.role_vote == "normal" else (1 / len(role)) 
    # find most voted role
    role_vote = sorted(role_vote.items(), key=lambda item: item[1], reverse=True)
    return role_vote[0][0]

def preprocess_samples_gsm8k(samples):
    prompts_in_chat_template = []
    for sample in samples:
        retrieved = sample["retrieved"]
        roles = [r[1] for r in retrieved]
        role = retrieval_role_vote(roles)
        ...
        message = [
                    ...
                    {"role": "user", "content": "Solve: " + sample["question"]}
                ]
...
save_name = args.infer_data_path.split("/")[-1]
