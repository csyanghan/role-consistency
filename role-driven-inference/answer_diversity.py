...
def process_sample(sample, data_name):
    ...
    if isinstance(sample_answers, list):
        ...
        return {
            "solution": gt_answer,
            "role_raw_answers": sample_answers[0],
            "role_answer": inner_pred_answer[0] if len(inner_pred_answer) > 0 else 0,
            "self_consistency_answers": inner_pred_answer if len(inner_pred_answer) > 0 else [],
            "self_consistency_count": calculate_answers_count_given_arr(inner_pred_answer) if len(inner_pred_answer) > 0 else 0,
            "self_consistency_diversity": ngram_diversity_score(sample_answers)
        }
    else:
...
if __name__ == "__main__":
    ...
    # 初始化 final_result 字典
    final_result = {role: {"sc_count_arr": [], "sc_diversity_arr": []} for role in actor_roles}
    print_final_result = {role: {"sc_count_average": 0, "sc_diversity_average": 0} for role in actor_roles}
    # 计算每个角色的准确率
    for role_idx in range(len(actor_roles)):
        ...
        print_final_result[actor_roles[role_idx]]["sc_count_average"] = np.mean(final_result[actor_roles[role_idx]]["sc_count_arr"])
        print_final_result[actor_roles[role_idx]]["sc_diversity_average"] = np.mean(final_result[actor_roles[role_idx]]["sc_diversity_arr"])
    ...
    print(print_final_result, flush=True)
